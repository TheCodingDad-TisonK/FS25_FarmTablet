...
FarmTabletUI:registerDrawer(FT.APP.ANIMALS, function(self)
    ...
    for _, pen in ipairs(pens) do
        ...
        local cardH = FT.py(10)
            + (pen.hasFood        and FT.py(24) or 0)
            + (pen.hasWater       and FT.py(24) or 0)
            + (pen.hasCleanliness and FT.py(24) or 0)
            + FT.py(18)  -- header row
        ...
        -- Food bar
        if pen.hasFood and pen.foodPct then
            ...
            self.r:appText(x + FT.px(8), y, FT.FONT.TINY,
                ...
                RenderText.ALIGN_LEFT, FT.C.TEXT_DIM)
            y = y - FT.py(10)
            y = self.r:progressBar(x + FT.px(4), y, cw - FT.px(8),
                pct, 100, bc)
        end

        -- Water bar
        if pen.hasWater and pen.waterPct then
        ...
        -- Cleanliness bar
        if pen.hasCleanliness and pen.cleanPct then
            local pct = math.max(0, math.min(100, pen.cleanPct))
            local bc  = barColor(pct)
            self.r:appText(x + FT.px(8), y, FT.FONT.TINY,
                string.format("CLEAN  %d%%", pct),
                RenderText.ALIGN_LEFT, FT.C.TEXT_DIM)
            y = y - FT.py(10)
            y = self.r:progressBar(x + FT.px(4), y, cw - FT.px(8),
                pct, 100, bc)
